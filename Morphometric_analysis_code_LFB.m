...
for xlink = 0.25%[0.25 0.5 0.75 1 1.25 1.5 1.75 2] %crosslinker number density (per um^2)
        xlinker_id = xlinker_id + 1;
        name = strcat('case_rest_xperiodic_100_11_',num2str(xlink),'_');
        list = get_directory_names(pwd,name); %return all files that match the filename given
        xl_mask_name = strcat('/Users/oakenuwa/Desktop/Projects/Andreas_morphometric_data/Morphometric_Actin_Masks/',num2str(xlink*xdim*ydim),'_XL_masks'); %Filaname for the skeletonized image
    ...
    for file = 1:5%length(list)
            ...
            for s = length(links)/tot_links %final timeframe of simulation
